...
class DisplayCallback(tf.keras.callbacks.Callback):
  def on_train_begin(self, logs=None):
      wandb.log({"Prediction": [wandb.Image((array_to_img(sample_image)), caption="Image"),
                                wandb.Image(drawer(array_to_img(sample_image), [sample_target]), caption="Base truth"),
                                wandb.Image(drawer(array_to_img(sample_image), [sample_target, targetize(self.model.predict(sample_image[tf.newaxis, ...]))]),
