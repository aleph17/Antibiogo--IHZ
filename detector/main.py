...
if __name__=="__main__":
    ...
    wandb.init(
        project="yolo",
        name= f"{'single_batch_test':<20}|{date.today()}",
        config={
            "epoch": EPOCHS
        },
    )
    ...
    model_history = model.fit(orig_train_batches.take(1),
                              epochs=config.epoch,
                              verbose = 0,
                              validation_data=vald_batches,
                              callbacks=[wandb_callbacks, keras_cv.callbacks.PyCOCOCallback(vald_batches, bounding_box_format="center_xywh")]#,savemodel_callback,DisplayCallback()]  #+EarlyStopping_callback
                              )
